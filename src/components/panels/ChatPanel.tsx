...
import type { AIMessage, HumanMessage, SessionCookie, DASessionState } from '@/types';
import { getSubmitLabel } from '@/lib/phases';
import { sendDAMessage, advanceDATab, startDASession } from '@/actions/student';
...
function AIBubble({ msg, session }: { msg: Message; session: SessionCookie }) {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-2`}>
      ...
      <div className={`max-w-[85%] px-3 py-2 rounded-lg text-sm leading-relaxed whitespace-pre-wrap ${isUser ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-800'}`}>
        {msg.content}
      </div>
      {isUser && (
...
export default function ChatPanel({
  ...
  const [messages, setMessages] = useState<Message[]>(() => {
    if (isChatbot) {
      return initialAIMessages.map((m) => ({
        source: 'ai' as const, role: m.role, content: m.content, id: m.id,
      }));
    }
    return initialHumanMessages.map((m) => ({
      source: 'human' as const, sender_id: m.sender_id, content: m.content, id: m.id, created_at: m.created_at,
    }));
  });
  ...
  async function handleSend() {
    ...
    if (isChatbot) {
      ...
      const res = await sendDAMessage(session.id, phase, text, summary, passageContent);
      ...
      if (res.error) {
        ...
        setMessages((prev) => [
          ...
          { source: 'ai', role: 'assistant', content: res.utterance, id: String(Date.now() + 1) },
        ]);
  ...
  // DA tab click (advance to next item)
  async function handleTabClick(idx: number) {
    if (idx <= activeTabIdx) return;
    const isUnlocked = daState?.resolutions[daState.priority_queue[idx - 1]];
    if (!isUnlocked) return;
    setLoading(true);
    await advanceDATab(session.id, phase);
    setActiveTabIdx(idx);
    setDaState((prev) => prev ? { ...prev, current_item_idx: idx, current_step: 1, item_identification_cumulative: false, item_verbalization_cumulative: false } : prev);
    setLoading(false);
  }
    ...
    return (
      <div className="h-full w-10 bg-white border border-slate-200 rounded-lg flex flex-col items-center py-3 gap-3 shrink-0">
        ...
        <span className="text-xs text-slate-300 [writing-mode:vertical-rl] select-none mt-2">
          {isChatbot ? 'AI 채팅' : '멘토 채팅'}
        </span>
      </div>
    );
  }

  const tabs = isChatbot && daState?.priority_queue?.length ? daState.priority_queue : [];
  ...
  return (
    <div className="h-full flex flex-col bg-white border border-slate-200 rounded-lg overflow-hidden shrink-0 w-80">
      ...
      {/* Chatbot-only item tabs (#1/#2/#3) */}
      {isChatbot && tabs.length > 0 && (
        <div className="flex border-b border-slate-200 shrink-0">
          {tabs.map((_, idx) => {
            const resolved = daState?.resolutions[tabs[idx - 1]] ?? (idx === 0);
            const isActive = idx === activeTabIdx;
            const isLocked = idx > activeTabIdx && !daState?.resolutions[tabs[idx - 1]];
            return (
              <button
                key={idx}
      ...
      {error && <p className="px-3 py-1.5 text-xs text-red-600 bg-red-50 border-t border-red-100">{error}</p>}
